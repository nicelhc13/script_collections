...
def run_bfs(property_graph: PropertyGraph, input_args, source_node_file):
    ...
    if not source_node_file == "":
        ...
        for source in sources:
            ...
            print(f"BFS source,", source, file=sys.stderr)
            print(f"BFS timer (s),",
                  round((timer_algo_end - timer_algo_start), 2),
                  file=timer_file)
            check_schema(property_graph, property_name)
            assert property_graph.get_node_property(property_name) \
                   [start_node].as_py() == 0
...
def run_sssp(property_graph: PropertyGraph, input_args, source_node_file):
    ...
    if not source_node_file == "":
        ...
        for source in sources:
            source = source.strip("\n")
            timer_algo_start = time.time()
            sssp(property_graph, int(source), edge_prop_name,
                 property_name, sssp_plan)
            timer_algo_end = time.time()
            print(f"SSSP source,",source, file=sys.stderr)
            ...
            sssp_assert_valid(property_graph, start_node,
                              edge_prop_name, property_name)
        ...
        timer_algo_start = time.time()
        sssp(property_graph, start_node, edge_prop_name,
             property_name, sssp_plan)
        timer_algo_end = time.time()
...
def run_jaccard(property_graph: PropertyGraph, input_args):
    ...
    similarities: np.ndarray = \
                  property_graph.get_node_property(property_name).to_numpy()
...
#pagerank_plan = PagerankPlan.pull_residual(tolerance, max_iteration, alpha)
    pagerank_plan = PagerankPlan.pull_topological(tolerance, max_iteration, alpha)
...
def run_bc(property_graph: PropertyGraph, input_args, source_node_file):
    ...
    if not source_node_file == "":
        ...
        for i in range(0, len(sources), n):
            ...
            timer_algo_start = time.time()
            betweenness_centrality(property_graph, property_name,
                                   sources_to_use, bc_plan)
            timer_algo_end = time.time()

            print(f"Between centrality source,",sources_to_use, file=sys.stderr)
            ...
            stats = BetweennessCentralityStatistics(property_graph,
                                                    property_name)
            print(f"STATS:\n",stats, file=sys.stderr)
            property_graph.remove_node_property(property_name)
    else:
...
def run_tc(property_graph: PropertyGraph, input_args):
    ...
    print(f"Triangle counting timer (s),",
          round((timer_algo_end - timer_algo_start), 2),
          file=timer_file)
    print(f"Number of Triangles,",n, file=sys.stderr)
...
def run_all_gap(args):
    ...
    if args.application in ["bfs", "sssp", "bc", "jaccard"]:
        ...
        print(
            f"[TIMER] Time to read propertyGraph : ",
            round((timer_graph_construct_end -
                    timer_graph_construct_start),2)
        ...
        print(
            f"[TIMER] Time to read propertyGraph : ",
            round((timer_graph_construct_end - timer_graph_construct_start),
            2)," seconds", file=sys.stderr
        )
        print(f"#Nodes: ",len(graph)," #Edges: ",graph.num_edges(),
              file=sys.stderr)
